...
impl<'a> SessionStore<'a> {
    ...
    pub async fn create(
        ...
        info!("Created session: {} for character: {}", id.as_ref(), character_id.as_ref());
    ...
    pub async fn delete(&self, character_id: &CharacterId, session_id: &SessionId) -> Result<()> {
        ...
        info!("Deleted session: {} for character: {}", session_id.as_ref(), character_id.as_ref());
    ...
    fn session_dir(&self, character_id: &CharacterId, session_id: &SessionId) -> PathBuf {
        self.storage.character_dir(character_id)
            .join("sessions")
            .join(&session_id.0)
    }
