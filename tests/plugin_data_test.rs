...
async fn test_plugin_kv_roundtrip() {
    ...
    let out = server
        .handle_plugin_kv_get(serde_json::json!({
    ...
    server
        .handle_plugin_kv_set(serde_json::json!({
            "plugin_name": "dice-roller", "key": "config",
...
async fn test_plugin_jsonl_append_and_read() {
    ...
    let out = server
        .handle_plugin_jsonl_read(serde_json::json!({
    ...
    for i in 0..3 {
        server
            .handle_plugin_jsonl_append(serde_json::json!({
                "plugin_name": "tracker", "file": "events.jsonl",
                "line_json": format!("{{\n  \"event\": {}\n}}", i)
            }))
            .await
...
async fn test_plugin_blob_text_and_base64_roundtrip() {
    ...
    // text write
    let out = server
        .handle_plugin_blob_write(serde_json::json!({
    ...
    // read back as text
    let out = server
        .handle_plugin_blob_read(serde_json::json!({
...
async fn test_plugin_resources_list_and_data() {
    ...
    let out = server
        .dispatch_resource("airp://plugins/stats/files")
        .await
        .unwrap();
