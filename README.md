...
> `/mcp/v1` 是标准 **MCP Streamable HTTP** 端点（spec 2025-06-18，基于 rmcp）：真实 JSON-RPC 派发、`initialize` 生命周期、会话 `Mcp-Session-Id`、`MCP-Protocol-Version` 校验、`application/json`↔`text/event-stream` 协商。任何 MCP 客户端（如 **[AIRP-Gateway](https://github.com/GhostXia/AIRP-Gateway)**）可直接对接。鉴权见[安全与部署](#安全与部署)。
...
- 所有数据存储在用户本地磁盘
- 通信走本机（stdio）或自部署 HTTP（默认 loopback；可选局域网，见[安全与部署](#安全与部署)）；不经任何第三方
