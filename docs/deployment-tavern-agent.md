...
**两个必须想清的坑**：
1. **对话历史归谁（防上下文重复）**：酒馆每轮 API 调用**天然带 `messages[]`**（它的可见记录）。真正要防的是**同一批对话被喂进模型上下文两次** —— 一次来自酒馆 `messages[]`、一次来自 AIRP `get_recent_context`。定「**实时上下文的单一来源**」：
   - **最简（酒馆为准）**：酒馆持有历史；AIRP 本模式**只供数据**（卡/预设/世界书/状态），不参与历史/记忆。
   - **要 AIRP 归档/记忆**：则**有意**把历史落到 AIRP（agent 以 `append_message` 作权威记录，或在 `seal_volume` 前批量写入）—— 这是**刻意持久化**，与「上下文重复」是两码事。关键只在：别让酒馆全量历史 + AIRP `get_recent_context` **同时**进同一上下文。
...
**配置要点**（酒馆侧）：空白角色 + 清空 system prompt + 关世界书/正则 + 直通预设（把 Story String / 格式模板设到最简，如仅 `{{mes}}`；或用空白预设）→ 它只转发剧情文字。**代价**：比直连模型慢（agy2api 假流式 + agent 往返 + AIRP 调用），且 agy2api 脆（见 §3）。
