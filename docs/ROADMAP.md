...
- **碰撞安全**（Gemini 提点）：删→建同名→再删 会撞 trash 条目。每次删除存入**碰撞安全的子项**（如 `.trash/{原相对路径}@{删除时间戳}` 或 uuid 段），绝不覆盖既有 trash 项。
- 清理：**purge 不得阻塞启动** —— stdio 由客户端拉起、`initialize` 有超时，启动时扫 `.trash` 可能拖垮 init（Gemini 提点）。优先**删除时顺带清超期** / 异步后台清 / 手动 `purge` 工具；**不做**启动期同步全扫。
